  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setStats({
          pendingBorrows: (borrows.data as Borrow[]).filter((b) => b.status === 'PENDING').length,
          overdueReturns: (borrows.data as Borrow[]).filter((b) => b.status === 'APPROVED' && b.dueDate && new Date(b.dueDate) < new Date()).length,
          unpaidFines: (fines.data as Fine[]).filter((f) => !f.isPaid).length,
