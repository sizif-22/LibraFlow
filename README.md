...
### Sprint Goal
> *"By the end of Sprint 1, the team can register, log in with role-based access, and view/search the book catalog."*
...
|---|---|---|---|
| US-01 | As a user, I want to register with my email and password so I can access the system | 3 | Backend |
| US-02 | As a user, I want to log in and receive a JWT token so my session is secure | 3 | Backend |
| US-03 | As a system, I want to enforce role-based access so each user sees only what they're allowed | 2 | Backend |
| US-04 | As a librarian, I want to add a new book to the catalog so students can borrow it | 3 | Backend |
| US-05 | As a librarian, I want to edit or delete a book so the catalog stays accurate | 2 | Backend |
| US-06 | As a student, I want to search for a book by title or author so I can find what I need | 3 | Backend |
| US-07 | As a user, I want to see a login and register page so I can authenticate | 3 | Frontend |
| US-08 | As a student, I want to see the book catalog with search so I can browse available books | 3 | Frontend |
| US-09 | As a librarian, I want a dashboard to add/edit books so I can manage the catalog | 3 | Frontend |

---

### Sprint 1 Tasks Breakdown

#### 🔵 Backend Tasks (Ahemd + Sayed)

```
Week 1 — Backend
├── Day 1-2: Project setup
│   ├── Initialize Bun + Elysia project
│   ├── Configure Prisma + MySQL connection
│   ├── Define initial schema (User, Book)
│   └── Setup Git repo + branching strategy
│
├── Day 2-3: Auth module (feature/auth)
│   ├── POST /api/auth/register
│   ├── POST /api/auth/login (returns JWT)
│   ├── JWT middleware for protected routes
│   └── Role-based guard middleware
│
└── Day 3-5: Book module (feature/book-management)
...
Week 1 — Frontend
├── Day 1: Project setup
│   └── Initialize React project (Next)
│
├── Day 2-3: Auth pages (feature/auth)
│   ├── /register page + form validation
│   ├── /login page + form validation
│   ├── Store JWT in memory/localStorage
│   └── Protected route component
│
└── Day 4-5: Book catalog pages (feature/book-management)
    ├── /books page — list + search bar
    ├── Book card component
    ├── /librarian/books — manage catalog
    └── Add/Edit book modal
```
...
> Use this every day in your team group chat:
...
- [ ] Code follows clean code principles (meaningful names, small functions)
